...
var videoStream = new VideoTranscodeStream( { highWaterMark: 16 * 1024 }, { source: '/dev/video0' } );
...
videoStream.on('transcode_start',function(data) {
	console.log(cli.redBright('< Transcoding START >'));
})

videoStream.on('transcodeend',function(data) {
	console.log(cli.yellowBright('< Transcoding STOP >'));
})
...
function streamVideoCamera(res, req, format) {
	...
	/*
		NOTE: 	Bug caused when you dont do as below. no data is sent to browser.

				this is probably a result of the changing way streams behave when using events
	*/

	videoStream.pipe(res);

	videoStream.on('data', function(chunk) {
		console.log(cli.blueBright('.'));
